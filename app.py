...
#                     st.info("Check your inbox and click the recovery link")
...
def auth_page():
    ...
    if "auth_mode" not in st.session_state:
        ...
        # ------------ JAVASCRIPT RECOVERY DETECTION ------------
    # This executes client-side JavaScript to grab the FULL URL including the '#' fragment
    url = st_javascript("window.location.href")

    # Ensure the JS has finished running and returned a string
    if url and isinstance(url, str):
        if "type=recovery" in url or "access_token" in url:
            st.session_state.auth_mode = "reset"
        elif "otp_expired" in url or "error_description" in url:
            st.error("⏰ The reset link from your email has expired! Please request a new one.")
    ...
    with col2:
        ...
        [data-testid="stForm"] {
            ...
            """,
                ...
                # =================================================
                if st.session_state.auth_mode == "reset":
                    st.markdown('<div class="title">Create New Password</div>', unsafe_allow_html=True)
                    st.markdown('<div class="subtitle">Enter your new password</div>', unsafe_allow_html=True)
                    ...
                    if st.button("Forgot Password?"):
                        if email:
                            ...
                            if response is not False:
                                ...
                                st.error("Failed to send reset link. is the email correct")
